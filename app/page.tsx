...
function UpsellBanner() {
  return (
    <div style={{ background: theme.dark, borderRadius: 4, padding: "28px 32px", marginBottom: 32, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 20 }}>
      <div>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, color: theme.cream, marginBottom: 6 }}>
...
const QUESTIONS = [
  { id: "spaName", label: "What is the name of your med spa?", placeholder: "e.g. Lumiere Aesthetics" },
...
function Intake({ onDone }: any) {
  ...
  const [limitReached, setLimitReached] = useState<{ message: string; calendlyUrl: string } | null>(null);
  const emailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim());
  const filled = emailValid && QUESTIONS.every(q => (answers[q.id] || "").trim());

  async function generate() {
    setLoading(true); setError(""); setLimitReached(null);
    try {
      ...
      if (res.status === 403 && sop.error === "limit_reached") {
        setLimitReached({ message: sop.message, calendlyUrl: sop.calendlyUrl });
        return;
      }
  ...
  if (limitReached) return (
    <div style={{ minHeight: "100vh", background: theme.cream }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "100px 32px", textAlign: "center" }}>
        <div style={{ fontSize: 40, color: theme.gold, marginBottom: 20 }}>✦</div>
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 32, fontWeight: 300, marginBottom: 16 }}>You've used your free procedure</h1>
        <p style={{ color: theme.muted, fontSize: 15, fontWeight: 300, lineHeight: 1.7, marginBottom: 32 }}>{limitReached.message}</p>
        <a href={limitReached.calendlyUrl} target="_blank" rel="noopener noreferrer" className="sb hl"
          style={{ display: "inline-block", padding: "16px 40px", borderRadius: 2, fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", textDecoration: "none", color: theme.dark }}>
    ...
    <div style={{ minHeight: "100vh", background: theme.cream }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "64px 32px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 500, marginBottom: 8 }}>Your email address</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@yourspa.com"
              style={{ width: "100%", padding: "12px 14px", border: `1.5px solid ${email && !emailValid ? theme.rose : email ? theme.gold : theme.border}`, borderRadius: 2, fontFamily: "'DM Sans',sans-serif", fontSize: 14, background: theme.card, outline: "none", fontWeight: 300 }} />
            <p style={{ fontSize: 11, color: theme.muted, marginTop: 6, fontWeight: 300 }}>One free procedure per email. We'll send your result here too.</p>
          </div>
          {QUESTIONS.map((q, i) => (
            <div key={q.id}>
...
function Portal({ generatedSop, spaName }: any) {
  ...
  const name = spaName || "Lumiere Aesthetics";
